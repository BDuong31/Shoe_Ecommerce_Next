...
export const getImages = async (dto: IConditionalImage): Promise<IApiResponse<IImage[]>> => {
    try {
        const { data } = await axiosInstance.get(endpoints.image.getImagesByProductId(dto.refId, dto.type, dto.isMain));
        return data;
    } catch (error) {
